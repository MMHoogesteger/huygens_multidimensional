...
load('data\sim_2_x_ref\2x_hamiltonian');


fou = s.results.analysis.fourier;
somega = fou.omega;
sa0 = fou.Fz0(1:3);
sa1 = fou.Fzkn(1:3,1);
sA1 = 2*abs(sa1);
sP1 = angle(sa1)+0.5*pi;
...
[G0s,Gs] = getTransfer_n_x(Nt,symomega,1,params);
...
fJ = @(rho) errDescnx(Nt,rho_f,G_func,dGdomega_func,params,s.U_gains(1:2),s.HStar(1:2),diag([1e3 1 1]),rho_ids,rho,1e3);
fJf = @(rho) errDescnx(Nt,rho_f,G_func,dGdomega_func,params,s.U_gains(1:2),s.HStar(1:2),diag([1e3 1 1]),rho_ids,rho,1e5);
...
subplot(6,1,2)
plot(omv,oAt1mi,'kx')
ylabel('$A_{1\theta_1}$ [rad]','Interpreter','Latex')
legend('hide')
grid on
subplot(6,1,3)
plot(omv,oAt2mi,'kx')
ylabel('$A_{1\theta_2}$ [rad]','Interpreter','Latex')
legend('hide')
grid on
subplot(6,1,4)
plot(omv,oPxmi,'kx')
ylabel('$\psi_{1x}$ [rad]','Interpreter','Latex')
legend('hide')
grid on
subplot(6,1,5)
...
plot(omv,oPt2mi,'kx')
ylabel('$\psi_{1\theta_2}$ [rad]','Interpreter','Latex')
legend('hide')
grid on
subplot(6,1,6)
...
legend('hide')
grid on
genTikz('2x_hbe_optim_omega_in.tex');
%% Anti-phase
figure;
subplot(6,1,1)
plot(omv,oAxmia,'kx')
