...
simSettings.name = ['tang_xyphi_' num2str(N) '_gamma'];
...
simSettings.varNameCell = {'gamma'};
perMat = unique(sort(nchoosek(repmat(0:11,1,N-1),N-1),2),'rows','stable').';
gammaMat = [zeros(1,size(perMat,2));perMat]*2*pi/12;
simSettings.varVecCell{1} = mat2cell(gammaMat,N,ones(1,size(perMat,2)));
