function [Jk,nablaJk] = harmonic_n_xyphi_eval_r_fft_with_grad(Nt,akm,omega,G_func,dGdomega_func,params,U_gains,HStar,R,gamma,psi,res)
bDebug = false;
Nq = Nt+3;
...
dFtotdA_fft = fft(dFtotdA,[],3)./length(t);
dFtotdP_fft = fft(dFtotdP,[],3)./length(t);
dFtotdomega_fft = fft(dFtotdomega,[],3)./length(t);
Ftotextra_fft = fft(Ftotextra,[],3)./length(t);
...
G = zeros(Nq,Nq,Nr);
dGdomega = zeros(Nq,Nq,Nr);

dGcdA = zeros(NA,NA);
dGcdP = zeros(NA,NA);
dGcdomega = zeros(NA,1);

for nr = 1:Nr
    ekc = exp(-1i*nr*omega*t);
    nids = (nr-1)*Nq+(1:Nq).';
    dcdA(nids,:) = dFtotdA_fft(:,:,nr+1);
    dcdP(nids,:) = dFtotdP_fft(:,:,nr+1);
    
    dcdomega(nids,1) = 1/omega*(Dk(:,nr)-Ftot(:,:,end).*ekc(end))...
            + omega/(2*pi)*trapz(dFtotdomega.*ekc - Ftot.*1i.*nr.*t.*ekc,3)*tstep;
    
    dcdomega(nids,1) = 1/omega*(Dk(:,nr)-Ftot(:,:,end).*ekc(end))...
            + dFtotdomega_fft(:,:,nr+1) -1i.*nr.*Ftotextra_fft(:,:,nr+1);
